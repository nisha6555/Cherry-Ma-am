...
async function generateContentWithRetry(params: { model: string; contents: any; config?: any }, retries = 5, initialDelay = 1500) {
  const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || "";
  if (!apiKey) {
    throw new Error("GEMINI_API_KEY is not configured in environment. Using fallback synthesis.");
  }
  let delay = initialDelay;
  const originalModel = params.model;
  const client = getAiClient();
  
  // Define sequence based on the starting model, including high-quota flash and flash-lite models
  let modelSequence = ["gemini-3.7-flash", "gemini-flash-latest", "gemini-3.1-flash-lite"];
  if (originalModel === "gemini-3.1-flash-lite") {
    modelSequence = ["gemini-3.1-flash-lite", "gemini-flash-latest", "gemini-3.7-flash"];
  } else if (originalModel === "gemini-flash-latest") {
    modelSequence = ["gemini-flash-latest", "gemini-3.7-flash", "gemini-3.1-flash-lite"];
  } else if (originalModel && originalModel !== "gemini-3.7-flash" && originalModel !== "gemini-3.1-flash-lite" && originalModel !== "gemini-flash-latest") {
    modelSequence = [originalModel, "gemini-flash-latest", "gemini-3.7-flash", "gemini-3.1-flash-lite"];
  }
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if ((isTransient || isServerError || isQuotaOrStatus429 || isNetworkError) && attempt < retries) {
        ...
        console.error(`[REST Server] API Call generated error on model "${params.model}" (Attempt ${attempt}/${retries}):`, errMsg || err);
        throw err;
      }
    }
  }
...
async function classifyAcademicDiscipline(markdown: string, filename?: string): Promise<string> {
  ...
  const mathKeywords = [
    "mathematics", "math", "maths", "real numbers", "polynomials", "quadratic", "linear equations",
    "arithmetic progression", "triangles", "coordinate geometry", "trigonometry", "circles", "surface area",
    "volume", "statistics", "probability", "theorem", "hypotenuse", "tangent", "sin", "cos", "tan",
    "algebra", "geometry", "calculus", "integral", "derivative", "matrix", "matrices", "\\frac", "\\sqrt",
    // Hindi keywords
    "गणित", "वास्तविक संख्याएं", "बहुपद", "समीकरण", "द्विघात", "समांतर श्रेढ़ी", "त्रिभुज", "निर्देशांक ज्यामिति",
    "त्रिकोणमिति", "वृत्त", "रचनाएं", "क्षेत्रफल", "आयतन", "सांख्यिकी", "प्रायिकता", "प्रमेय"
  ];
  ...
  const scores = [
    ...
    { subject: "Physics", score: physScore },
    { subject: "Biology", score: bioScore },
    { subject: "Mathematics", score: mathScore },
  ...
  // Fast Gemini Lite text classifier (if API key is available)
  const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || "";
  if (apiKey) {
    try {
      console.log("[Subject Classifier] Running lightweight Gemini Lite subject classifier...");
      const snippetText = markdown.substring(0, 3000);
      const subjectCall = await generateContentWithRetry({
        model: "gemini-3.1-flash-lite",
        contents: {
          parts: [{
            text: `Analyze the educational document title "${filename || "Notes"}" and content snippet below to determine its exact core academic subject discipline.\n` +
                  "Options: 'Chemistry', 'Physics', 'Biology', 'Mathematics', 'History', 'Geography', 'Civics', 'Economics', 'Computer Science', 'English', 'Hindi'.\n" +
                  "Return ONLY the exact single capitalized subject name. Do not write explanation or sentences.\n\nContent snippet:\n" + snippetText
          }]
        }
      });
      ...
      console.warn("[Subject Classifier] Notice during Gemini Lite classification:", err?.message || err);
    }
  }
  ...
  try {
    ...
    if (markdown) {
      ...
      if (chapterMatch && chapterMatch[1]) {
        ...
        if (rawH.length > 2 && !rawH.toLowerCase().startsWith("file_") && !rawH.toLowerCase().startsWith("slide_") && !/^\d+$/.test(rawH)) {
          detectedTitle = rawH;
    ...
    let normalizedSubject = await classifyAcademicDiscipline(markdown, filename);

    // Chemistry override if chemical formulas or reactions are explicitly present in the markdown
    if (markdown.match(/(\bNH_?3\b|ammonia|haber|hydrochloric|nitric|sulfuric|acid|base|salt|bond|reaction|organic|element|periodic|equilibrium|titration|catalyst|oxidation|reduction|covalent|ionic|valency|molar mass|vapour density)/i)) {
      normalizedSubject = "Chemistry";
    }

    if (!detectedTitle) {
      detectedTitle = normalizedSubject === "Chemistry" ? "Study of Compounds: Chemical Fundamentals" : "Academic Study Notes";
    }
    ...
    sessionState.activeDocument = {
      ...
      mimeType,
      markdown,
      mode: isSocraticMode ? "socratic" : isMistakeMode ? "mistake" : isDoubtMode ? "doubt" : isCheatSheetMode ? "cheatsheet" : isPYQMode ? "pyq" : "explain",
...
app.post("/api/battle-room/extract-syllabus", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to extract syllabus and generate battle questions: " + err.message });
  }
});
...
app.post("/api/problem-guide", async (req, res) => {
  try {
    ...
    const systemPrompt = `You are Cherry Ma'am 🧭 acting as an expert Socratic AI Problem Guide for school students (${studentGradeStr}, ${studentBoardStr}).
...
- Maintain a warm, friendly, peer-like tone as Cherry Ma'am throughout!`;
    ...
    res.status(500).json({ error: "Problem Guide service error: " + err.message });
  }
});
...
app.post("/api/generate-concept-infographic", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to generate concept infographic: " + err.message });
  }
});
...
app.post("/api/pyq-analyze-8020", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to analyze 10-Year PYQ: " + err.message });
  }
});
...
app.post("/api/pyq-marking-heatmap", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to generate Weightage Heatmap: " + err.message });
  }
});
...
app.post("/api/pyq-predicted-paper", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to generate Predicted Exam Paper: " + err.message });
  }
});
